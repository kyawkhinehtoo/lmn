           ...
           <div class="relative flex flex-wrap ">
            ...
            <button @click="openModal"  class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring focus:ring-gray-300 disabled:opacity-25 transition">
              Create</button>
                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg"  v-if="sale_persons.data">
                 ...
                 <input class="w-half rounded py-2 my-3 float-right" type="text" placeholder="Search Township" v-model="search" v-on:keyup.enter="searchTsp">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="row in sale_persons.data " v-bind:key="row.id">
                      ...
                      <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                        ...
                        <form @submit.prevent="submit">
                          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="">
                                  <div class="mb-4">
                                      <label for="name" class="block text-gray-700 text-sm font-bold mb-2">Sale Person Name:</label>
                                      <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="name" placeholder="Enter SalePerson Name" v-model="form.name">
            ...
            <span v-if="sale_persons.links">
                <pagination class="mt-6" :links="sale_persons.links"  />
            </span>
            </div>
        </div>
    </app-layout>
    ...
    setup(props){
      ...
      const form = reactive({
        ...
        function submit() {
           if(! editMode.value){
            ...
            onError: (errors) => {
                ...
                Inertia.post('/sale_person/' + form.id, form,{
                onSuccess: (page) => {
                closeModal()
                resetForm()
                Toast.fire(
        ...
        function deleteRow(data) {
                ...
                data._method = 'DELETE';
                Inertia.post('/sale_person/' + data.id, data)
                closeModal()
                resetForm()
        }
        function openModal() {
        ...
        const searchTsp = () =>{
          ...
          Inertia.get('/sale_person/', {sale_person : search.value}, { preserveState: true })
        }
