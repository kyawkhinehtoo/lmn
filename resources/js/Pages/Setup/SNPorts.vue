      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="(row, index) in sns.data" v-bind:key="row.id">
                  ...
                  <td class="px-6 py-3 whitespace-nowrap">{{ dns.filter((d) => d.id == row.dn_id)[0].name_port }}</td>
        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg" v-if="overall" v-show="tab == 1">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="(row, index) in overall" v-bind:key="row.id">
                <td class="px-6 py-3 whitespace-nowrap">{{ index + 1 }}</td>
                <td class="px-6 py-3 whitespace-nowrap">{{ dns.filter((d) => d.id == row.dn_id)[0].name_port }}</td>
                <td class="px-6 py-3 whitespace-nowrap">{{ row.name }}</td>
      ...
      <div>
        <div class="mt-4 text-sm">
          <div class="mt-1 flex rounded-md shadow-sm" v-if="dns.length !== 0">
            <multiselect deselect-label="Selected already" :options="dns" track-by="id" label="name_port" v-model="form.dn_id" :allow-empty="true"></multiselect>
          </div>
...
export default {
  ...
  setup(props) {
    ...
    onMounted(() => {
      props.dns.map(function (x) {
        return (x.name_port = `${x.name}/${x.port}`);
      });
    });
    onUpdated(() => {
      props.dns.map(function (x) {
